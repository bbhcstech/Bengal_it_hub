...
@php
    $person = collect($event['people'])->first(fn ($p) => $p[0] === 'Chief Adviser');
@endphp
        ...
        @if($person)
            <div class="mt-10">
                @include('partials.hackfest-person-card', [
                    'person' => $person,
                    'variant' => 'feature',
                    'roleLabel' => 'The Chief Adviser',
                    'extraIntro' => 'As the Chief Adviser, he is providing strategic guidance on program direction, partnerships, and high-level decision-making, ensuring alignment with the event\'s goals.',
                ])
            </div>
        @endif
