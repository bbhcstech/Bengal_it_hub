...
<body class="bih-admin grid min-h-screen place-items-center px-5 py-10">
    <div class="w-full max-w-5xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 lg:grid lg:grid-cols-[1fr_.9fr]">
        ...
        <form method="POST" action="{{ route('admin.authenticate') }}" class="p-8 md:p-10">
            ...
            @if($errors->any())
                <div class="mt-5 rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-900" role="alert">{{ $errors->first() }}</div>
            @endif

            <label class="mt-6 block text-sm font-black" for="email">Email</label>
            <input class="mt-2 w-full rounded border border-slate-300 px-4 py-3" id="email" type="email" name="email" value="{{ old('email') }}" required autofocus>

            <label class="mt-4 block text-sm font-black" for="password">Password</label>
            <input class="mt-2 w-full rounded border border-slate-300 px-4 py-3" id="password" type="password" name="password" required>

            <label class="mt-4 flex items-center gap-2 text-sm font-bold" for="remember">
                <input type="checkbox" id="remember" name="remember" value="1"> Remember this device
            </label>
