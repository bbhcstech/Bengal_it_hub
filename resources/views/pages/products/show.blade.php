...
<section class="bih-product-show-hero relative overflow-hidden bg-slate-950 text-white">
...
<section class="bih-section bg-white">
    <div class="bih-container grid gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-start">
        ...
        <div class="bih-card bih-product-tech-panel p-6">
            <p class="text-xs font-black uppercase tracking-wide text-teal-700">Technologies Used</p>
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach($technologies as $tech)
                    <span class="bih-product-tech-chip rounded-full border border-slate-200 bg-slate-50 px-3.5 py-2 text-xs font-black text-slate-700">{{ $tech }}</span>
                @endforeach
            </div>
            <div class="mt-6 grid gap-3 border-t border-slate-100 pt-5">
                <a class="bih-button bih-product-tech-action bih-product-tech-action-primary justify-center" href="/contact?interest={{ urlencode($product['title']) }}">Talk to the Team</a>
                <a class="bih-product-tech-action bih-product-tech-action-website inline-flex min-h-11 items-center justify-center gap-2 rounded-md border-2 border-sky-700 px-4 py-3 font-extrabold text-sky-700 transition hover:bg-sky-700 hover:text-white" href="{{ $product['website'] ?? url('/') }}" target="_blank" rel="noopener">Visit Product Website</a>
                <a class="bih-product-tech-action bih-product-tech-action-linkedin inline-flex min-h-11 items-center justify-center gap-2 rounded-md border-2 border-teal-700 px-4 py-3 font-extrabold text-teal-700 transition hover:bg-teal-700 hover:text-white" href="{{ $siteBrand['socials']['LinkedIn'] ?? config('bengalhub.brand.socials.LinkedIn') }}" target="_blank" rel="noopener">Follow on LinkedIn</a>
            </div>
        </div>
    </div>
</section>
                ...
                @foreach($otherProducts as $other)
                    <article class="bih-card bih-image-card flex flex-col overflow-hidden">
                        ...
                        <div class="flex flex-1 flex-col p-5">
                            ...
                            <a class="bih-product-more-link mt-4" href="{{ route('products.show', $other['slug']) }}">See More &rarr;</a>
                        </div>
                    </article>
                @endforeach
            </div>
