...
@php
    $adminNav = [
        ['label' => 'Dashboard', 'route' => 'admin.dashboard', 'match' => 'admin.dashboard', 'icon' => 'DB', 'can' => true],
        ['label' => 'Pages', 'route' => 'admin.pages', 'match' => 'admin.pages*', 'icon' => 'PG', 'can' => auth()->user()->canManage('pages')],
        ...
        ['label' => 'Settings', 'route' => 'admin.settings', 'match' => 'admin.settings*', 'icon' => 'ST', 'can' => auth()->user()->canManage('settings')],
    ];
@endphp
<div class="bih-admin-shell">
    ...
    <section class="bih-admin-panel">
        ...
        <main class="bih-admin-content">
            @if(session('status'))
                <div class="mb-5 rounded-lg border border-teal-200 bg-teal-50 p-4 font-bold text-teal-900">{{ session('status') }}</div>
            @endif
            @if($errors->any())
                <div class="mb-5 rounded-lg border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-900">{{ $errors->first() }}</div>
            @endif
            @yield('content')
        </main>
    </section>
